...
class DepositRequestTable extends React.Component {
  static propTypes = {
    ...
    return (
      <div className="coin-title">
        ...
        <div className="visible-xs visible-sm coin-amount">
          <p className="bold">
            <FormattedMessage id="app.history.deposit.coin.amount.title" />
          </p>
          <span>{formatNumber(row.requested_amount)}</span>
        </div>
      </div>
    );
  }
  ...
  addressFormatter(cell, row) {
    ...
    if (row.coin !== GLOBAL_VARIABLES.BASE_CURRENCY) {
      if (type === GLOBAL_VARIABLES.WALLET_TYPE.DEPOSIT) {
        switch (row.status) {
          case WALLET_ORDER_STATUS.SUCCESS:
            if (row.tx_id) {
              transactionStatus = (
                <div className="tx-id line-break">
                  {' '}
      ...
      addressSection = (
        <div>
          <div className="address-value line-break">
            <strong>
      ...
      <div>
        ...
        <div className="col-xs-12 p-0 text-left">
          <div className="col-xs-6">
            <p className="title">
              <FormattedMessage id="app.history.deposit.coin.title" />: <b>{row.coin_id.toUpperCase()}</b>
            </p>
          </div>
          <div className="col-xs-6">
            <p className="title">
              <FormattedMessage id="app.history.deposit.coin.amount.title" />: <b>{formatNumber(row.requested_amount)}</b>
            </p>
          </div>
        </div>
      </div>
    ...
    return (
      ...
      >
        ...
        <TableHeaderColumn
          ...
          dataField="status"
          dataAlign="center"
          className="status-col-header hidden-xs hidden-sm"
          columnClassName="status-col hidden-xs hidden-sm"
          dataSort
        >
        ...
        <TableHeaderColumn
          ...
          // width="100px"
          dataField="coin_id"
          className="coin-col-header hidden-xs hidden-sm"
          columnClassName="coin-col hidden-xs hidden-sm"
          dataSort
        >
          <FormattedMessage id="app.history.deposit.coin.title" />
        </TableHeaderColumn>
...
export default injectIntl(DepositRequestTable);
